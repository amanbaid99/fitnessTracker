      ...
      <div className="flex gap-3 p-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <p className="min-w-0 flex-1 truncate font-semibold text-ft-text">
              {exercise.name}
            </p>
            {logged && (
              <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-ft-success/10 px-2 py-0.5 text-[11px] font-medium text-ft-success">
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
            <span className="rounded-full bg-ft-bg px-2 py-0.5 text-[11px] font-medium text-ft-muted">
            ...
            {alternates.length > 0 && (
              <span className="inline-flex items-center gap-1 rounded-full bg-ft-accent/10 px-2 py-0.5 text-[11px] font-medium text-ft-accent">
          ...
          {record ? (
            <p className="mt-1.5 flex flex-wrap items-center gap-x-1.5 text-xs">
              <span className="inline-flex items-center gap-1 font-medium text-ft-warning">
              ...
              {target && <span className="text-ft-accent">· {target}</span>}
            </p>
          ) : (
            <p className="mt-1.5 text-xs text-ft-muted">
              No record yet — log a set to start tracking {active.name}.
            </p>
          )}

          {!logged && lastPerformedName && lastPerformedName !== exercise.name && (
            <p className="mt-1 flex items-center gap-1 text-xs text-ft-muted">
      ...
      {onLog && (
        <>
          <button
            ...
            onClick={() => setOpen((v) => !v)}
            aria-expanded={open}
            className="flex w-full items-center justify-between border-t border-ft-border/70 px-4 py-2.5 text-sm font-medium text-ft-accent transition-colors hover:bg-ft-accent/[0.04]"
          >
            {logged
          ...
          {open && (
            <div className="space-y-3 border-t border-ft-border/70 px-4 py-3">
              {options.length > 1 && (
                <div>
                      ...
                      >
                        ...
                        {i === 0 && (
                          <span
                            className={cn(
                              "text-[10px]",
                              i === selected ? "text-white/70" : "text-ft-muted",
                            )}
                          >
                            main
                          </span>
              ...
              <div>
                <div className="flex items-center gap-2 px-1 pb-1">
                  <span className="w-7 text-[11px] font-medium text-ft-muted">Set</span>
                  <span className="flex-1 text-[11px] font-medium text-ft-muted">
                    Weight (kg)
                  </span>
                  <span className="flex-1 text-[11px] font-medium text-ft-muted">Reps</span>
                  <span className="w-8" />
                </div>
                ...
                <ul className="space-y-1.5">
                  {sets.map((set, i) => (
                    <li key={i} className="flex items-center gap-2">
                      <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-ft-bg text-xs font-semibold text-ft-muted">
                        {i + 1}
                      </span>
                      <Input
                        type="number"
                      ...
                      <button
                        ...
                        className="flex size-8 shrink-0 items-center justify-center rounded-md text-ft-muted transition-colors hover:bg-ft-bg hover:text-ft-danger disabled:opacity-30"
                      >
                ...
                <button
                  type="button"
                  onClick={addSet}
                  className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-xl border border-dashed border-ft-accent/50 py-2 text-sm font-medium text-ft-accent transition-colors hover:bg-ft-accent/[0.05]"
                >
